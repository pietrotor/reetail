...
export const transformBaseReportGetData = (
  data: CraCheckReportBaseReportGetResponse
) => {
  ...
  return report.items.flatMap((item) =>
    item.accounts.map((account) => {
      ...
      const averageInflow =
        accountInsights?.average_inflow_amounts?.pop()?.total_amount;
      const averageOutflow =
        accountInsights?.average_outflow_amounts?.pop()?.total_amount;
      return {
        accountName: account.name,
        averageDaysBetweenTransactions:
          accountInsights?.average_days_between_transactions?.toFixed(2),
        averageInflowAmount: formatCurrency(
          averageInflow?.amount,
          averageInflow?.iso_currency_code
        ),
        averageOutflowAmount: formatCurrency(
          averageOutflow?.amount,
          averageOutflow?.iso_currency_code
        ),
        averageBalance: formatCurrency(
          account.balances.average_balance,
          account.balances.iso_currency_code
        ),
...
export const transformIncomeInsightsData = (
  data: CraCheckReportIncomeInsightsGetResponse
) => {
  ...
  return [
    {
      ...
      historicalAnnualIncome: formatCurrency(
        historicalIncome?.amount,
        historicalIncome?.iso_currency_code
      ),
      forecastedAnnualIncome: formatCurrency(
        forecastedIncome?.amount,
        forecastedIncome?.iso_currency_code
      ),
    },
  ] as Array<CreditInsightsGetItem>;
};

export const transformPartnerInsightsData = (
  data: CraCheckReportPartnerInsightsGetResponse
) => {
